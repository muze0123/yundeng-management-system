...
4. `claude.md` 决定 HTML 原型技术栈、导航、Mock 数据和 Portal 标注实现方式。
...
**本期包含**：左侧导航 + 顶部栏 + 主内容区布局、静态分组标题、可点击一级菜单、Router Outlet、侧栏收起与响应式行为、全局通知和账户入口、Toast / Dialog / Drawer 容器、Loading / Empty / Error / Forbidden 状态、Mock 会话与导航数据、框架/业务 Portal 标注隔离，以及工作台和已完成业务模块的一次性迁移（含财务管理 / 发票管理）。
...
- App Shell 不自动创建“页面工具”区块，也不得把业务标注开关移入工具栏；历史内容模块根级 `.app-module-toolbar` 统一不展示。业务标注开关恢复为页面右侧悬浮胶囊按钮，支持拖拽调整位置、视口安全区限制与位置持久化；拖拽完成不得触发标注显隐切换。页面专属操作由业务模块放置在自身内容区内。TopBar 左侧固定展示平台 Logo 与平台名称，并承载移动导航；右侧保留框架标注、通知和账号入口，不显示当前功能模块名称。
- App Shell 与 Router Outlet 均锁定在视口高度内，只有主内容区承担纵向滚动；业务标注浮标固定于视口，不随主内容滚动离开可视区，也不得与 TopBar 的全局操作混用。
