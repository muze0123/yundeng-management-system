...
**页面布局（强制）**：根目录 `index.html` 是云登平台所有模块页面的唯一系统框架入口。除 `登录.html` 外，所有后台页面必须通过 `index.html#route-id` 复用该框架，仅在内容承载区开发模块内容，不得自行重建或分叉系统壳层。
...
> `index.html` 通过 `Prototype/系统框架.js` 加载完整菜单交互逻辑。新页面复用框架后直接继承，不得重复实现或修改公共行为。
>
>
> | 操作           | 行为                      |
> | ------------ | ----------------------- |
...
> **核心规则**：根目录 `index.html` 与 `Prototype/系统框架.js` 是侧边栏与路由配置的唯一基准。新增、改名、删除模块或调整菜单结构时，必须同步更新根入口、相关页面入口及全部路由引用，确保导航结构与实际页面一致。

| 变更场景 | 同步范围 | 操作 |
|----------|----------|------|
| **新建 HTML 原型页面** | 根 `index.html`、`Prototype/系统框架.js`、对应模块脚本 | ① 在侧边栏添加对应菜单项与 `index.html#route-id`；② 在 `Prototype/modules/` 注册内容模块 |
| **修改模块名或 HTML 文件名** | 根 `index.html`、`Prototype/系统框架.js`、对应模块脚本 | ① 更新菜单名称及路由；② 全局搜索旧名称，更新 JS 跳转、`a` 标签等引用；旧业务跳转壳不再新增 |
| **删除模块或 HTML 原型页面** | 根 `index.html`、`Prototype/系统框架.js`、对应模块脚本 | ① 移除对应菜单项与路由；② 全局搜索并清理失效引用 |
| **调整菜单层级/结构** | 根 `index.html`、`Prototype/系统框架.js` | 同步更新菜单结构、路由配置、菜单初始化逻辑及权限树等硬编码引用，**不可遗漏** |

**同步检查清单**（每次变更后必过）：

- [ ] `index.html` / `Prototype/系统框架.js` — 侧边栏与路由已更新（唯一基准入口）
