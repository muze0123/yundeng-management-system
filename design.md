...
| 侧栏菜单 | 14px / 400–600 | 静态分组标题、一级菜单、二级菜单、折叠态二级 Flyout 与底部系统菜单 |
...
| 列表工具栏 | 标题行左侧为语义列表标题，当前结果总数紧邻标题右侧（间距 8px）；列表工具栏采用左对齐弹性布局，总数与标题之间无空格阻断；下一行左侧为状态 Tab，右侧放刷新、导出等仅作用于当前列表的操作；分割线位于状态 Tab 上方。没有状态 Tab 时，分割线直接位于标题统计行与指标卡或表格之间 |
...
**筛选条件展开/收起 UI 与交互**：代理订单、用户列表等筛选区统一使用 `.filter-toggle` 文字入口，位于重置右侧；原生 `button type="button"` 仅用于点击和键盘语义，不套用 `.btn` / `.btn-default` 按钮外观。透明背景、无边框、无阴影、无水平内边距，高度 32px；文字为 13px / 500、默认 `ink-sub`，左侧 Lucide 箭头为 16×16px，图文间距 6px。hover 与键盘 `focus-visible` 时文字和箭头同步高亮为 `primary`，背景保持透明；移出后恢复默认色，焦点使用 2px primary 轮廓。收起态显示“展开”与向下箭头，展开态显示“收起”与向上箭头；支持点击、Enter 和空格切换，同步 `aria-expanded`，通过 `aria-controls` 指向筛选条件容器。切换不提交查询、不清空任何筛选值、不改变当前列表结果；操作组始终可见，筛选区高度随内容自然增减，不裁切或覆盖后续区块。

**代理订单默认折叠规则**：默认仅展示当前筛选栅格的第一行，展开后显示全部条件；窗口尺寸变化时按当前栅格重新计算第一行，不固定写死字段数量。重置恢复默认筛选并收起。
...
│   └─ 二级菜单 .menu-item.l2：不显示图标，文字起点与一级菜单文字左对齐（当前 App Shell padding-left 36px）
...
- 侧栏仅一级菜单显示 Lucide 功能图标，统一为 16×16px；二级菜单及侧栏折叠态二级 Flyout 不显示功能图标，也不保留图标占位，仅保留文字；折叠按钮、一级菜单 Chevron、顶部栏与业务内容图标按各自组件规范执行；
- 侧栏所有文字（静态分组标题、一级菜单、唯一展开组、二级菜单、折叠态二级 Flyout 与底部系统菜单）统一使用 14px，字距为 0；二级菜单不额外缩进，文字起点与一级菜单文字左对齐；长分组按实际内容高度展开，超出侧栏可视区时自动滚动到完整可操作范围，禁止被底部系统菜单裁切；
- 收起态二级 Flyout 必须限制在视口安全区内，超出可用高度时仅在 Flyout 内滚动，不得溢出视口或遮挡顶部栏；
- 根目录 `index.html` 为唯一后台框架入口，TopBar、Sidebar、全局通知和账号入口只维护一份，业务模块加载到 Router Outlet；导航统一使用英文 `page` 查询参数。
- 已完成业务模块以 `Prototype/modules/*.js` 交付业务内容、样式与脚本；允许共享本地 JS/CSS，旧中文 HTML 仅作兼容跳转。
- 工作台是无 page 参数或 `?page=home` 的默认内容；切换模块必须支持浏览器前进/后退，并在会话内恢复模块筛选值、当前 Tab、分页和有效滚动位置，侧栏折叠态使用本地持久化；
